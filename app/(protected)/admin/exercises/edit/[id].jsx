...
export default function EditExercise() {
  ...
  const { data: initialData, isLoading: isLoadingExercise } = useQuery({
  ...
  const form = useForm({
    defaultValues: {
      ...
      local_image_uri: initialData?.local_image_uri || "",
      cloudinary_image_public_id: initialData?.cloudinary_image_public_id || "",
      local_video_uri: initialData?.local_video_uri || "",
      cloudinary_video_public_id: initialData?.cloudinary_video_public_id || "",
      instructions: initialData?.instructions || "",
      is_unilateral: initialData?.is_unilateral === 1,
    },
    onSubmit: async ({ value }) => {
      try {
        ...
        Toast.show({
          type: "success",
          text1: "¡Éxito!",
          text2: "El ejercicio fue actualizado correctamente.",
          position: "bottom",
        });
        ...
        Toast.show({
          type: "error",
          text1: "Error al actualizar, intente nuevamente.",
          text2: error.message,
          position: "bottom",
        });
      }
    ...
    >
      ...
      <View className="px-4 pt-6 pb-2">
        ...
        <Text className="text-sm font-manrope text-ui-text-muted dark:text-ui-text-mutedDark mt-1">
          Actualiza los datos del ejercicio en tu catálogo.
        </Text>
      </View>
          ...
          validators={{
            ...
            onChangeAsync: async ({ value }) => {
              ...
              if (existing.length > 0 && existing[0].id !== id) {
                return "Ya existe otro ejercicio con este nombre.";
              }
              return undefined;
            },
          }}
              ...
              <View ref={uploadVideoCardRef}>
                ...
                >
                  <InputUploadVideo
                    value={field.state.value}
                    onChange={field.handleChange}
                    setVideoPublicId={(vid) =>
                      form.setFieldValue("cloudinary_video_public_id", vid)
                    }
