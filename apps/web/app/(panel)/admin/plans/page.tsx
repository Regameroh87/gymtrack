...
// Planes de entrenamiento (admin). Clon de apps/mobile admin/plans/index.web.jsx:
// lee training_plans + conteo de plan_assignments activas del gym activo, stats,
// búsqueda + filtro por nivel, grid de cards (2 col) con badges.
...
export default function PlansListPage() {
  ...
  const stats = useMemo(() => {
    ...
    return { total: plans.length, active: assignments?.length || 0, levels: levels.size };
  }, [plans, assignments]);

  const levels = useMemo(() => {
    if (!plans) return [];
    const set = new Set(plans.map((p) => p.level).filter(Boolean) as string[]);
  ...
  return (
    <div className="p-4 pb-14 md:p-9">
      ...
      <div className="mb-6 flex flex-col gap-3.5 md:flex-row">
        <StatCard icon={ClipboardList} label="Total" value={stats.total} iconColor="#0284c7" bubble="bg-sky-50" />
        <StatCard icon={Users} label="Asignaciones activas" value={stats.active} iconColor={brandPrimary[600]} bubble="bg-brandPrimary-50" />
        <StatCard icon={SlidersHorizontal} label="Niveles únicos" value={stats.levels} iconColor="#7c3aed" bubble="bg-violet-50" />
      </div>
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        ...
        <div className="mb-3 flex flex-wrap items-center gap-1.5">
          {lvl && (
            <div className={`flex items-center gap-1 rounded-md px-2 py-0.5 ${lvl.bubble}`}>
          ...
          <div className="flex items-center gap-1 rounded-md bg-brandPrimary-50 px-2 py-0.5">
            <Calendar size={10} color={brandPrimary[600]} />
            <span className="font-manrope text-[10px] font-bold uppercase tracking-wider text-brandPrimary-600">
          ...
          {genderBadge && (
            <div className="flex items-center gap-1 rounded-md border border-brandPrimary-500/25 bg-brandPrimary-500/10 px-2 py-0.5">
              <span className="font-manrope text-[10px] font-bold uppercase tracking-wider text-brandPrimary-600">
                {genderBadge}
              </span>
            </div>
        ...
        {/* Footer */}
        <div className="mt-auto flex items-center justify-between border-t border-ui-input-border pt-2">
          <div className="flex items-center gap-1">
            <Users size={11} color={ui.text.muted} />
            <span className="font-manrope text-[10px] text-ui-text-muted">
              {activeAssignments} asignación{activeAssignments === 1 ? "" : "es"} activa{activeAssignments === 1 ? "" : "s"}
            </span>
          </div>
          <span className="font-manrope text-[10px] text-ui-text-muted">
